...
def train_epoch(model, loader, criterion, optimizer, scaler, device):
    ...
    for images, targets in tqdm(loader, desc="Training CoordAttn", leave=False):
...
def main():
    ...
    print("\n--- Initializing Proposed Coord-EfficientNet-B3 Model ---")
    model = CoordEfficientNetB3(pretrained=True).to(device)
    ...
    epochs = 10
    best_acc = 0.0
    best_model_path = os.path.join(models_dir, "efficientnet_b3_coord_8020.pth")
    history = {'train_loss': [], 'train_acc': [], 'val_loss': [], 'val_acc': []}

    print(f"\n--- Starting 10-Epoch Training for Proposed Coord-EfficientNet-B3 ---")
    ...
    print(f"\nCoordinate Attention Training Complete in {total_time/60:.2f} minutes. Best Accuracy: {best_acc:.2f}%")
    ...
    plt.title('Confusion Matrix - Coord-EfficientNet-B3 (80:20 Split)')
    plt.colorbar()
    ...
    plt.tight_layout()
    cm_path = os.path.join(models_dir, "confusion_matrix_coord_8020.png")
    ...
    plt.title('Loss Curves - Coord-EfficientNet-B3')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
    ...
    plt.title('Accuracy Curves - Coord-EfficientNet-B3')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.legend()
    plt.grid(True)
    curves_path = os.path.join(models_dir, "training_curves_coord_8020.png")
    plt.tight_layout()
    plt.savefig(curves_path, dpi=300)
    plt.close()

    print("\n" + "="*76)
    print("   MODEL 4: PROPOSED COORD-EFFICIENTNET-B3 - FINAL EVALUATION REPORT")
    print("="*76)
    ...
    print("\n--- 4-WAY COMPREHENSIVE BENCHMARK (BATCH SIZE = 8) ---")
    print(f"{'Metric':<20} | {'Base (B=8)':<14} | {'CBAM (B=8)':<14} | {'Triplet (B=8)':<16} | {'CoordAttn (B=8)':<16}")
    print("-" * 88)
    print(f"{'Overall Accuracy':<20} | {'98.10%':<14} | {'97.90%':<14} | {'98.05%':<16} | {f'{final_acc:.2f}%':<16}")
    print(f"{'Precision':<20} | {'0.9859':<14} | {'0.9858':<14} | {'0.9878':<16} | {f'{prec:.4f}':<16}")
    print(f"{'Recall':<20} | {'0.9760':<14} | {'0.9720':<14} | {'0.9730':<16} | {f'{rec:.4f}':<16}")
    print(f"{'F1-Score':<20} | {'0.9809':<14} | {'0.9789':<14} | {'0.9804':<16} | {f'{f1:.4f}':<16}")
    print(f"{'ROC-AUC Score':<20} | {'0.9944':<14} | {'0.9960':<14} | {'0.9958':<16} | {f'{auc:.4f}':<16}")
    print(f"{'Parameters':<20} | {'10.91 M':<14} | {'11.20 M':<14} | {'10.91 M':<16} | {f'{total_params/1e6:.2f} M':<16}")
    print("="*88)
