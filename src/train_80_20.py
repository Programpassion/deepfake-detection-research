...
def build_80_20_loaders(data_dir, total_per_class=5000, batch_size=8, seed=42):
    ...
    - Stratified random 80:20 split:
        - 80% (8,000 images: 4,000 Real, 4,000 Fake) -> Training
        - 20% (2,000 images: 1,000 Real, 1,000 Fake) -> Test / Validation
    """
...
def main():
    ...
    epochs = 10
    best_acc = 0.0
    best_model_path = os.path.join(models_dir, "efficientnet_b3_base_8020.pth")
    history = {'train_loss': [], 'train_acc': [], 'val_loss': [], 'val_acc': []}

    print(f"\n--- Starting 10-Epoch Training on 80:20 Split ---")
    ...
    plt.title('Confusion Matrix - 80:20 Internal Split (Base Paper Setup)')
    plt.colorbar()
    ...
    plt.tight_layout()
    cm_path = os.path.join(models_dir, "confusion_matrix_base_8020.png")
    ...
    plt.title('Loss Curves (80:20 Split)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
    ...
    plt.title('Accuracy Curves (80:20 Split)')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.legend()
    plt.grid(True)
    curves_path = os.path.join(models_dir, "training_curves_base_8020.png")
    plt.tight_layout()
    plt.savefig(curves_path, dpi=300)
    plt.close()

    print("\n" + "="*72)
    print("       80:20 SPLIT REPLICATION - FINAL EVALUATION REPORT")
    print("="*72)
    ...
    print("\n--- DIRECT COMPARISON: BASE RESEARCH PAPER vs. OUR 80:20 REPLICATION ---")
    print(f"{'Metric':<25} | {'Base Paper Reported (Table 4)':<30} | {'Our 80:20 Replication':<20}")
    print("-" * 80)
    print(f"{'Overall Accuracy':<25} | {'97.95%':<30} | {f'{final_acc:.2f}%':<20}")
    print(f"{'Precision':<25} | {'0.9800':<30} | {f'{prec:.4f}':<20}")
    print(f"{'Recall':<25} | {'0.9800':<30} | {f'{rec:.4f}':<20}")
    print(f"{'F1-Score':<25} | {'0.9800':<30} | {f'{f1:.4f}':<20}")
    print(f"{'True Positives (Fake)':<25} | {'988 (out of 1009)':<30} | {f'{tp} (out of 1000)':<20}")
    print(f"{'True Negatives (Real)':<25} | {'971 (out of 991)':<30} | {f'{tn} (out of 1000)':<20}")
    print(f"{'ROC-AUC Score':<25} | {'Not reported':<30} | {f'{auc:.4f}':<20}")
    print("="*80)
