...
def train_epoch(model, loader, criterion, optimizer, scaler, device):
    ...
    for images, targets in tqdm(loader, desc="Training Triplet", leave=False):
...
def main():
    ...
    print("\n--- Initializing EfficientNet-B3 + Triplet Attention Model ---")
    model = EfficientNetB3Triplet(pretrained=True).to(device)
    ...
    epochs = 10
    best_acc = 0.0
    best_model_path = os.path.join(models_dir, "efficientnet_b3_triplet_8020.pth")
    history = {'train_loss': [], 'train_acc': [], 'val_loss': [], 'val_acc': []}

    print(f"\n--- Starting 10-Epoch Training for EfficientNet-B3 + Triplet Attention ---")
    ...
    print(f"\nTriplet Training Complete in {total_time/60:.2f} minutes. Best Accuracy: {best_acc:.2f}%")
    ...
    plt.title('Confusion Matrix - EfficientNet-B3 + Triplet Attention (80:20 Split)')
    plt.colorbar()
    ...
    plt.tight_layout()
    cm_path = os.path.join(models_dir, "confusion_matrix_triplet_8020.png")
    ...
    plt.title('Loss Curves - EfficientNet-B3 + Triplet Attention')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
    ...
    plt.title('Accuracy Curves - EfficientNet-B3 + Triplet Attention')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.legend()
    plt.grid(True)
    curves_path = os.path.join(models_dir, "training_curves_triplet_8020.png")
    plt.tight_layout()
    plt.savefig(curves_path, dpi=300)
    plt.close()

    print("\n" + "="*76)
    print("   MODEL 3: EFFICIENTNET-B3 + TRIPLET ATTENTION - FINAL EVALUATION REPORT")
    print("="*76)
    ...
    print("\n--- 3-WAY COMPARISON: BASELINE vs. CBAM vs. TRIPLET ATTENTION ---")
    print(f"{'Metric':<25} | {'Baseline (B=8)':<18} | {'CBAM (B=8)':<18} | {'Triplet Attention (B=8)':<22}")
    print("-" * 90)
    print(f"{'Overall Accuracy':<25} | {'98.10%':<18} | {'97.90%':<18} | {f'{final_acc:.2f}%':<22}")
    print(f"{'Precision':<25} | {'0.9859':<18} | {'0.9858':<18} | {f'{prec:.4f}':<22}")
    print(f"{'Recall':<25} | {'0.9760':<18} | {'0.9720':<18} | {f'{rec:.4f}':<22}")
    print(f"{'F1-Score':<25} | {'0.9809':<18} | {'0.9789':<18} | {f'{f1:.4f}':<22}")
    print(f"{'ROC-AUC Score':<25} | {'0.9944':<18} | {'0.9960':<18} | {f'{auc:.4f}':<22}")
    print(f"{'Parameters':<25} | {'10.91 M':<18} | {'11.20 M (+2.7%)':<18} | {f'{total_params/1e6:.2f} M (+0.003%)':<22}")
    print("="*90)
